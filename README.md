> [!IMPORTANT]
> **本插件已并入 [`dsh-usage`](https://github.com/nanami-0713/dsh-usage)** —— 更完善的 Token 消耗体系**集成插件**：全局用量看板（本插件的后续）+ 会话级实时费用徽标 + Coding Plan 订阅额度，三位一体。
>
> 新功能与维护都在 `dsh-usage` 进行，建议直接安装它。本仓库保留作历史参考：你的计价配置与全量索引缓存会被 `dsh-usage` 首次启动时自动迁移（旧文件改名保留、缓存复制不移动），`/api/dsh-usage-board/*` 在过渡期内由 `dsh-usage` 别名兼容（config API 自动做 v1↔v2 视图转换）。
...
| Kimi K3 | 不分时 | 输入 $3.0 · 缓存命中 $0.30 · 输出 $15.0 | Moonshot AI 官方刊例（日志名 `kimi-k3` 与 Coding Plan 渠道的裸名 `k3` 命中同一条规则） |
